...
window.onload = function()
{
    ...
    var servietteh = new Image();
    servietteh.src = "image/servietteh.png";

    var Player = servietteh;

    /*player touché*/

     var serviettet = new Image();
    serviettet.src = "image/serviettet.png";

     var servietter = new Image();
    servietter.src = "image/servietter.png";

    var servietterip = new Image();
    servietterip.src = "image/servietterip.png";

    var fonsde = new Image();
    fonsde.src = "image/fonsde.png";

    var Playertouche = ['servietter','serviettet'];


     /*les objet qui descendent du ciel */

     var bang = new Image();
    bang.src = "image/bang32.png";

     var bebecrack = new Image();
    bebecrack.src = "image/bebecrack32.png";
    ...
    {        
        ...
        if(pv >0){
            ...
            calculeColisiion();
                    ...
                    move = 346;        
        context.beginPath();
        context.drawImage(fonsde, animatefonsde*280, 0, 280, 344, 100, 20, 600, 600);
        context.fill();
        vitesseAnim++;
        if(vitesseAnim>2){
        animatefonsde++;
...
acceleration=Math.floor((calculeTime()-debutTemps)/10000000);
        ...
        if(pv <=0){
            bonus[i].statut=false;
                        ...
                        if(bonus[i].statut){
            if(bonus[i].gender==1){
         context.drawImage(bebecrack, 0, 0, 50, 50, bonus[i].coord[0], bonus[i].coord[1], 50, 50);

            }else{
             context.drawImage(bang, 0, 0, 50, 50, bonus[i].coord[0], bonus[i].coord[1], 50, 50);
        ...
        context.beginPath();
        if(pv ==3)
        context.drawImage(servietteh, 0, 0, 312, 436, move, 355, 104, 145);
        if(pv ==2)
        context.drawImage(serviettet, 0, 0, 312, 436, move, 355, 104, 145);
        if(pv ==1)
        context.drawImage(servietter, 0, 0, 312, 436, move, 355, 104, 145);
        if(pv <=0)
        context.drawImage(servietterip, 0, 0, 312, 436, move, 355, 104, 145);
